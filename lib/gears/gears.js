 ...
 *
 *  @param {String} s   The 'pattern' from which to create URLs
 *  @param {Object} o   The object which contains values for the 'pattern'
 *  @param {Boolean} b  Whether or not the Gears can engage
 *  @return {Boolean}
 *
 *  @description
 *
 ...
 *  and there is no default so it must be set in the consuming application or else ... boom
 */
let pattern
...
export class Gears {
  ...
  static engage (s, o = {}) { // console.log('(Gears)engage()', s, o)
    let b = !!s // true if pattern is truthy true
    if (b) return engage(s, o)
    return b
  }

  static part = (s) => { // console.log('(Gears)part()', s)
    let S = map.get(s)
    if (S) return S
    map.set(s, S = part(s))
    return S
  ...
  static path = (s, o = {}) => ( // console.log('(Gears)path()', s)
